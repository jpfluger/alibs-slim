...
func TestUID(t *testing.T) {
	t.Run("NewUID", func(t *testing.T) {
		did := NewUID()
		assert.False(t, did.IsNil(), "NewUID should not be nil")
		assert.True(t, did.Valid, "NewUID should be valid")
	})

	t.Run("ParseUID Valid", func(t *testing.T) {
		validUUID := autils.NewNullUUIDWithValue()
		did := ParseUID(validUUID.UUID.String())
	...
	t.Run("ParseUID Invalid", func(t *testing.T) {
		did := ParseUID("invalid-uuid")
...
func TestUIDs(t *testing.T) {
	t.Run("Contains", func(t *testing.T) {
		...
		assert.False(t, dids.Contains(NewUID()), "UIDs should not contain a different UID")
	})
	...
	t.Run("IsValid", func(t *testing.T) {
		...
		assert.False(t, dids.IsValid(did1, did2, NewUID()), "Should return false if any UID in the input is missing")
	})
	...
	t.Run("Filter", func(t *testing.T) {
		...
		dids := UIDs{did1, did2}
		filtered := dids.Filter(func(did UID) bool {
			return did == did1
		})
