...
// ISON_PAGES_HTML_RELOAD_ON_RENDER if true, re-parses templates from FS on each render (dev/hot-reload); else preloads.
var ISON_PAGES_HTML_RELOAD_ON_RENDER = false

// ISON_PAGES_HTML_LOG_ERROR enables logging of errors during HTML template execution.
var ISON_PAGES_HTML_LOG_ERROR = false

// FSPagesHTML is an FS-based HTML/template renderer for the Echo framework.
type FSPagesHTML struct {
	...
	layoutsFS fs.FS                         // FS for layouts (optional: e.g., fs.Sub(embedweb.Files, "pages-html/layouts")).
	viewsFS   []fs.FS                       // FS for views (multiple sources: e.g., fs.Sub(embedweb.Files, "pages-html/views")).
}

// NewFSPagesHTML creates a new FSPagesHTML instance, loading templates from the provided FS sources.
func NewFSPagesHTML(layoutsFS fs.FS, viewsFS []fs.FS, funcMap *template.FuncMap) (*FSPagesHTML, error) {
	...
	templates, err := fph.preloadTemplates()
	if err != nil {
		return nil, fmt.Errorf("failed to preload templates: %w", err)
	}
	fph.templates = templates

	// If reload enabled, clear preloaded map to force re-parse on each render.
	if ISON_PAGES_HTML_LOADBYFILE || ISON_PAGES_HTML_RELOAD_ON_RENDER {
		fph.templates = make(map[string]*template.Template)
	}
...
func (t *FSPagesHTML) preloadTemplates() (map[string]*template.Template, error) {
...
func (t *FSPagesHTML) loadFilesFromFS(fsys fs.FS, ext string, optional bool) (map[string]string, error) {
...
func (t *FSPagesHTML) Render(w io.Writer, name string, data interface{}, c echo.Context) error {
	if ISON_PAGES_HTML_LOADBYFILE || ISON_PAGES_HTML_RELOAD_ON_RENDER {
...
func (t *FSPagesHTML) renderReload(name string, data interface{}, w io.Writer) error {
	...
	if ISON_PAGES_HTML_LOG_ERROR && executeErr != nil {
		alog.LOGGER(alog.LOGGER_APP).Error().Err(executeErr).Msg("exe html template reload")
	}
	return executeErr
}

// RenderPage renders a page to a string.
func (t *FSPagesHTML) RenderPage(name string, data interface{}) (string, error) {
