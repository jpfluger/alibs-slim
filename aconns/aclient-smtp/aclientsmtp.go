...
// Test validates the AClientSMTP, initializes if necessary, and tests the connection.
...
// test is the internal, lock-free version of Test.
func (cn *AClientSMTP) test() (bool, aconns.TestStatus, error) {
	...
	if err := cn.testConnectionWithCtx(ctx); err != nil {
		...
		//alog.LOGGER(alog.LOGGER_APP).Warn().Err(err).Msg("SMTP test failed")
		return false, aconns.TESTSTATUS_FAILED, err
	}
...
// Init initializes the SMTP client by validating the configuration and testing the connection.
func (cn *AClientSMTP) Init() error {
	cn.mu.Lock()
	defer cn.mu.Unlock()
	return cn.init()
}
...
func (cn *AClientSMTP) init() error {
	...
	case AUTHTYPE_SENDER_PLAIN:
		cn.auth = smtp.PlainAuth("", cn.Username, cn.Password, cn.Host)
	case AUTHTYPE_SENDER_IDENTITY:
		cn.auth = smtp.PlainAuth(cn.Identity, cn.Username, cn.Password, cn.Host)
	}
...
// testConnectionWithCtx tests the connection to the SMTP server using a provided context.
func (cn *AClientSMTP) testConnectionWithCtx(ctx context.Context) error {
	done := make(chan error, 1)
	go func() {
		client, err := smtp.Dial(cn.address)
		if err != nil {
			done <- err
			return
		}
...
// Refresh resets the initialization state, forcing re-init on next use.
func (cn *AClientSMTP) Refresh() error {
	cn.mu.Lock()
	defer cn.mu.Unlock()
	cn.hasInitialized = false
	return nil
