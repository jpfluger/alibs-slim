...
// CheckSnippetsLoaded checks if the snippets are loaded in the FSTEMPLATES.
// Returns an error if any required snippet is not loaded.
func (mt *MailTemplate) CheckSnippetsLoaded() error {
	if mt.hasSnippetText {
		if err := atemplates.FSTEMPLATES().GetFSSnippetTextIsLoaded(mt.SnippetTextName); err != nil {
			return err
		}
	}

	if mt.hasSnippetHTML {
		if err := atemplates.FSTEMPLATES().GetFSSnippetHTMLIsLoaded(mt.SnippetHTMLName); err != nil {
			return err
		}
	}
...
func (mt *MailTemplate) SendWithRenderOptions(smtpAuth ISMTPAuth, addressGroup *MailAddressGroup, subjectMerge []interface{}, dataBody interface{}) error {
	...
	if mt.hasSnippetText {
		output, err := atemplates.FSTEMPLATES().RenderSnippetsText(mt.SnippetTextName, dataBody)
		if err != nil {
	...
	if mt.hasSnippetHTML {
		output, err := atemplates.FSTEMPLATES().RenderSnippetsHTML(mt.SnippetHTMLName, dataBody)
		if err != nil {
